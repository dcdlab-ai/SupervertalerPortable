...
ЗАКРЫТО (25.09.2026, follow-up S2.1 — ГЕЙТ ПЕРЕД S2.2 СНЯТ): раздел 1.4 отчёта Stage 1 характеризовал все 7 duck-typed вызовов `load_general_settings` как `getattr(self, 'load_general_settings', lambda: {})()`, аудит Б1 оспаривал это для `Supervertaler.py:1809` и `modules/quicktrans.py:302-303`. ТРЕТЬЯ ПРОВЕРКА СДЕЛАНА — прямая цитата кода: формы `getattr(self, …, lambda: {})()` НЕТ НИ В ОДНОМ из 7 мест, везде `hasattr`-guard + обычное обращение к атрибуту — `Supervertaler.py:1803–1815` (`while parent: … elif hasattr(parent, 'load_general_settings'): settings = parent.load_general_settings()`), `modules/quicktrans.py:300–305` (`if hasattr(self.parent_app, 'load_general_settings'): settings = self.parent_app.load_general_settings()`), а также `7881`, `55559`, `55568` (`self.load_general_settings() if hasattr(self, 'load_general_settings') else {}`) и `64711`/`64726` (`if mw and hasattr(mw, 'load_general_settings'):`). АУДИТ Б1 ПОДТВЕРЖДЁН, формулировка Stage 1 ОШИБОЧНА. Дословные цитаты и полный инвентарь: `docs/refactoring/audits/batch7s21_open_item_quotes.txt`; разбор — §8.2 отчёта `docs/refactoring/reports/ОТЧЁТ Batch #7 Stage 2 (S2.1).txt`. СЛЕДСТВИЕ: делегат с исходным именем сохраняет все 7 мест работоспособными (hasattr=True → вызов уходит в сервис), «мягкого» duck-typing-контракта не требуется → S2.2 проектируется по строгой схеме делегатов, как S2.1.
...
#### Статус Step 7 после Batch #7 Stage 2 (S2.1) — 2026-09-25: ядро API перенесено, Step 7 НЕ закрыт
Выполнен под-батч **S2.1** (промпт — `docs/refactoring/prompts/Promt - EXTRACTION BATCH #7 STAGE 2 (S2.1 settings core).txt`): 6 методов ядра API / 39 строк (44639–44682) перенесены в новый **`modules/settings_service.py`** (класс `SettingsService`, 113 строк; конструктор — ЯВНЫЙ `settings_dir` + опц. `log`, без ConfigManager — вариант V1(б)). В монолите на их месте — 6 ТОНКИХ ДЕЛЕГАТОВ с исходными именами/сигнатурами (44639–44696): без них не работает `main()` 68370 (`window._load_settings_section("ui")`), getattr-обращения `modules/voice_tab.py` (1061/1062) и `modules/clipboard_manager_widget.py` (2284/2298/2299), а также ещё не перенесённые `_migrate_settings_to_unified` (→ `_get_settings_dir`) и `_migrate_voice_dictation_default_off` (→ `_load/_save_settings_section`). Дополнительно: `self.settings_service` создаётся в `__init__` (6301–6309) и **перепривязывается первым оператором `_reinitialize_with_new_data_path()`** (6668–6676) — `user_data_path` переприсваивается в 6630/6934/22859, и все три места ведут в этот метод; без перепривязки (или при ней в конце метода) делегаты и миграции при смене каталога данных работали бы со старым каталогом.
Измерения: монолит 68 386 → **68 400** строк (`git diff --numstat` 47/33, один файл); py_compile OK (144 файла); 8 метрик 1211/18/35/24/24/0/85/2 — без изменений; SHA256-манифест 143 → 144 (changed=1 — `Supervertaler.py`; added=1 — `modules/settings_service.py`; removed=0); перенос тел: 5/6 VERBATIM + 1 задокументированная инъекция пути (`_get_settings_dir` → `self.settings_dir`). Живой offscreen-прогон приложения — **13/13 PASS** (main():68370; getattr-пути voice_tab/clipboard; save-путь General-вкладки; полный цикл save→файл→load→окно на КОПИИ user_data; перепривязка при смене каталога; «4 вызова = 4 открытия settings.json» — кэша нет, семантика §1.9 сохранена). Смоук save/load — OK (`SAVE_OK`/`LOAD_OK`); Access Violation 0xC0000005 на teardown offscreen-процесса воспроизводится ИДЕНТИЧНО на HEAD (worktree 043da2a) → предсуществующий, вне скоупа S2.1.
NOT-MOVE: `modules/termbase_entry_editor.py:121/131/134` — файл побайтово как в HEAD; поведение (AttributeError, проглоченный try/except) не тронуто, сайты в фан-ин не считаются. Поправка инвентаря «`_save_unified_settings` fan-in = 12» подтверждена измерением (текстовых вызовов `_save_unified_settings(` ровно 12).
Отчёт: `docs/refactoring/reports/ОТЧЁТ Batch #7 Stage 2 (S2.1).txt`; аудиты `docs/refactoring/audits/batch7s21_*`, в т.ч. ПЕРЕСЧИТАННЫЕ AST-границы остатка Stage 2 — `batch7s21_remaining_ast.txt` (после S2.1 все документированные номера сдвинуты). Непокрытые проверки — раздел 4 отчёта.
СЛЕДУЮЩИЙ ПОД-БАТЧ: **S2.2 «general + общий reading» — 4 метода / 153 строки** (`_load_general_settings_from_file` 44897–44936, `save_general_settings` 44938–44943, `load_general_settings` 44542–44638 [SPLIT], `load_clipboard_privacy_settings` 44704–44713; `save_clipboard_privacy_settings` 44715–44736 остаётся в монолите — V3). ОТКРЫТЫЙ вопрос по duck-typed сайтам ЗАКРЫТ (см. абзац ЗАКРЫТО выше — аудит Б1 подтверждён прямой цитатой, форма `getattr(self, …, lambda)` отсутствует); перед стартом — только пересчитать границы AST заново (после S2.1 они уже пересчитаны: `docs/refactoring/audits/batch7s21_remaining_ast.txt`).

#### Статус Step 7 после Batch #7 Stage 2 (S2.2) — 2026-09-26: general + clipboard reading перенесены, Step 7 НЕ закрыт
Выполнен под-батч **S2.2** (промпт — `docs/refactoring/prompts/Promt - EXTRACTION BATCH #7 STAGE 2 (S2.2 general).txt`): 3 метода / 56 строк перенесены в `modules/settings_service.py` (113 → 197 строк) — `load_clipboard_privacy_settings` (44704–44713; в сервисе 140–149), `_load_general_settings_from_file` (44897–44936; 151–190), `save_general_settings` (44938–44943; 192–197). Тела — ВЕРБАТИМ (3/3, построчное сравнение со снапшотами HEAD-блоба, sha256 тел совпадают); новых параметров конструктора не потребовалось: единственные внешние зависимости тел — `_load_settings_section` / `_save_settings_section` / `log`, уже имеющиеся в сервисе (AST-замер §1.3 отчёта). В монолите — 3 тонких делегата на месте тел: 44711–44719, 44903–44909, 44911–44916 (сигнатуры байт-в-байт, 3/3 forwards+signature_identical).
Измерения: монолит 68 400 → **68 373** строки (`git diff --numstat` 23/50; difflib-замер — ровно 5 неравных регионов, все заявленные: комментарии +4/+3, делегаты −1/−33/0, сумма −27 == фактическому изменению файла); py_compile OK (144 файла); 8 метрик 1211/18/35/24/24/0/85/2 — без изменений; SHA256-манифест 144 → 144 (changed = РОВНО 2: `Supervertaler.py` 20419b6a… → 80993e2f…, `modules/settings_service.py` 4c683141… → 02914e1f…; added=0, removed=0). Живой offscreen-прогон приложения — **9/9 PASS**: делегат == сервис по значению; все 4 потребителя `_load_general_settings_from_file` (load_general_settings 44545, _toggle_match_panel_tm_layout 41560, get_effective_import_options 44315, _create_file_types_settings_tab 44507) работают через делегат; getattr-путь `modules/clipboard_manager_widget.py:771` на РЕАЛЬНОМ `ClipboardManagerWidget` действительно находит метод (инструментированный перехватчик: загрузчик вызван, не провал в None) и подхватывает МАРКЕР из файла-копии (`capture_enabled: false`), неизвестные ключи отфильтрованы по `DEFAULT_PRIVACY`; полный цикл save→файл→load→окно→виджет на КОПИИ user_data (api_keys и «чужая» секция сохранены; 0 предупреждений в логе); продовый settings.json за прогон побайтово не изменился; «3 вызова = 3 открытия» — кэша нет. Смоук save/load — OK (`SAVE_OK 6584`, `LOAD_OK S22Smoke 7`, `REACHED_END`), ДО/ПОСЛЕ на worktree HEAD 25627c9 одинаково (exit 0 в обоих).
БАГ СОХРАНЁН БИТ-В-БИТ: делегат `save_general_settings` оставлен БЕЗ default у обязательного `settings` (AST `defaults = []`) → вызов без аргумента падает тем же `TypeError`, что и в HEAD-worktree (текст совпадает посимвольно). НОВОЕ измерение: оба баговых сайта (теперь 66753 и 66796, класс `SuperlookupTab`) защищены `hasattr(<окно>, 'general_settings')`, а такого атрибута в монолите НЕТ НИ ОДНОГО присваивания → путь мёртв, баг спит (кандидат в VALIDATION_BACKLOG, пред-существующий, решение владельца).
NOT-MOVE: `load_general_settings` 44542–44638 (97 строк) — код не тронут вообще (byte-identical снапшоту HEAD); V3-требование SPLIT выполнено тем, что тело осталось в монолите, а его внутренний вызов `self._load_general_settings_from_file()` ушёл в делегат без правок тела. `save_clipboard_privacy_settings` (44721–44742) тоже остался в монолите целиком (V3).
Отчёт: `docs/refactoring/reports/ОТЧЁТ Batch #7 Stage 2 (S2.2).txt`; аудиты `docs/refactoring/audits/batch7s22_*` (ниже в отчёте есть раздел «Непокрытые проверки», в т.ч. N4 — баг, N14 — контролируемые касания продовых user-data).
СЛЕДУЮЩИЙ ПОД-БАТЧ: **S2.3 «LLM / proxy / provider / api keys» — 10 методов / 150 строк** (`load_llm_settings`, `save_llm_settings`, `load_proxy_settings`, `save_proxy_settings`, `_get_proxy_url`, `_get_proxy_dict`, `load_provider_enabled_states`, `save_provider_enabled_states`, `load_api_keys`, `save_api_keys`; fan-in ~116, из них ~10 вызовов из `modules/` плюс ~10 строковых обращений, часть имён приватные). Это РОВНО 10 методов — граница конвенции живого чек-листа ≤10, отметить в отчёте S2.3 явно. Границы вывести AST заново на дереве S2.2 (после сдвига −27 документированные номера неверны).
